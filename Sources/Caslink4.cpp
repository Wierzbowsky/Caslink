...
//  Podrezov	07.09.2022   Fixed the BIN chunk size for CRC calculation in both main code and single/multifile BIN loaders (thanks to Romsey for the bug report)
//  Podrezov	07.09.2022   Converted all MAC sources to ASM to be used with AS80 assembler by Frank Kingswood
//  Podrezov	09.05.2023   Massive rework of loaders to be able to work on computers with RAM in slot 0 or with 32kb (or less) RAM equipped with the RAM mapper
//******************************************************************************
...
void EncodeSingleFile(void)
{
	...
	else if ((unsigned char)InputMemPointer[0] == 0xfe && InputFileLength <= 24712L)
	{
	...
	if ((FileType == 2 || FileType == 3) && InputFileLength <= 24712L)
	{
	...
	if (FileType == 2 || FileType == 3)
	{
		...
		AddressBuffer[0] = 0;
		AddressBuffer[1] = 0xC1;
		a = (size_t)((size_t)sizeof(BINROMPreLoader) + (size_t)0xC100 - 1);
		AddressBuffer[2] = (char)a;
		AddressBuffer[3] = (char)(a >> 8);
		AddressBuffer[4] = 0;
		AddressBuffer[5] = 0xC1;
	...
	else if (FileType == 2)
	{
		...
		for (i = FileOffset; i < (RomEnd-RomBegin) + FileOffset + 1; i++) RomCRC = RomCRC + InputMemPointer[i];		// +1 byte correction!
		...
		AddressBuffer[0] = 0;
		AddressBuffer[1] = 0x81;
		a = (size_t)((size_t)sizeof(BINLoader) + (size_t)InputFileLength + (size_t)0x8100 - FileOffset - 1);
		AddressBuffer[2] = (char)a;
		AddressBuffer[3] = (char)(a >> 8);
		AddressBuffer[4] = 0;
		AddressBuffer[5] = 0x81;
   ...
   // Fill WAV header
	...
	for (l = 2; l < k; l++)
	{
		...
		else
		{
			...
			RomStart = (unsigned short)((unsigned char)InputMemPointer[6] * 0x100 + (unsigned char)InputMemPointer[5]);
			if (RomEnd <= RomBegin || RomStart >= RomEnd || RomStart < RomBegin) ExitOnError(25);
			for (i = FileQueue[l-2].FileOffset; i < (RomEnd-RomBegin) + FileQueue[l-2].FileOffset + 1; i++) RomCRC = RomCRC + InputMemPointer[i];		// +1 byte correction!
